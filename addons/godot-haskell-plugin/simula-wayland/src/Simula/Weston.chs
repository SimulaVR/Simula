...
{#pointer *weston_headless_backend_config as WestonHeadlessBackendConfigPtr -> WestonHeadlessBackendConfig #}

data WestonHeadlessBackendConfig = WestonHeadlessBackendConfig
  { headlessBaseConfig :: WestonBackendConfig
  , headlessUsePixman :: Bool
  } deriving (Show, Eq, Ord)


instance Storable WestonHeadlessBackendConfig where
  sizeOf _ = {#sizeof weston_headless_backend_config#}
  alignment _ = {#alignof weston_headless_backend_config#}
  peek ptr = WestonHeadlessBackendConfig
             <$> peek (plusPtr (castPtr ptr) {#offsetof weston_headless_backend_config->base#})
             <*> ((toEnum . fromIntegral) <$> {#get weston_headless_backend_config->use_pixman#} ptr)
  poke ptr WestonHeadlessBackendConfig{..} = do
    poke (plusPtr (castPtr ptr) {#offsetof weston_headless_backend_config->base#}) headlessBaseConfig
    {#set weston_headless_backend_config->use_pixman#} ptr (fromIntegral . fromEnum $ headlessUsePixman)
...
westonHeadlessBackendConfigVersion :: CUInt
westonHeadlessBackendConfigVersion = {#const WESTON_HEADLESS_BACKEND_CONFIG_VERSION #}
...
foreign import ccall "wrapper" createFlushDamageFunc :: (WestonSurface -> IO ()) -> IO (FunPtr (WestonSurface -> IO ()))

setFlushDamageFunc :: WestonCompositor -> FunPtr (WestonSurface -> IO ()) -> IO ()
setFlushDamageFunc = {#set weston_compositor->renderer->flush_damage #}