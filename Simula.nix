...
let
    ...
    devBuildFalse = ''
      ...
      echo "PATH=${xwayland}/bin:${xkbcomp}/bin:\$PATH LD_LIBRARY_PATH=${SDL2}/lib:${vulkan-loader}/lib \$(./utils/GetNixGL.sh) ${godot}/bin/godot.x11.opt.64 -m 2>&1 | ${coreutils}/bin/tee output.file" >> $out/bin/simula
    ...
    devBuildTrue = ''
      ...
      echo "if [ ! -d .import ]; then PATH=${xwayland}/bin:${xkbcomp}/bin:\$PATH LD_LIBRARY_PATH=${SDL2}/lib:${vulkan-loader}/lib \$(./utils/GetNixGL.sh) ${godot}/bin/godot.x11.tools.64 -e; else PATH=${xwayland}/bin:${xkbcomp}/bin:\$PATH LD_LIBRARY_PATH=${SDL2}/lib:${vulkan-loader}/lib \$(./utils/GetNixGL.sh) ${godot}/bin/godot.x11.tools.64 -m; fi" >> $out/bin/simula
      chmod +x $out/bin/simula
     '';
