...
This script does a few things under the hood: (i) checks whether you have ~nix~ and ~cachix~ installed (and, if not, installs them); (ii) checks which graphics drivers you have and (iii) builds Simula via ~nix~ and ~cachix~ with the appropriate driver flags. Simula is untested on machines with AMD drivers, though AMD cards running mesa drivers should be supported.
