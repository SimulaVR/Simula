...
Simula is a VR window manager for Linux that runs on top of [[https://godotengine.org/][Godot]].
...
Simula can be built and ran with ~nix~ as follows:

#+BEGIN_SRC shell
git clone --recursive https://github.com/SimulaVR/Simula
cd Simula

# Compile Simula (takes a while)
nix-build --option build-use-sandbox false \
          default.nix \
          --argstr driverCheck "$(./utils/DriverCheck.sh)" # Ensures Simula is compatible on any Linux machine with nvidia drivers, intel drivers, or systems running NixOS.

# Launch Simula
./result/bin/simula # Needs SteamVR to be running
#+END_SRC

Simula is untested on machines with AMD drivers, though AMD cards running intel drivers should be supported. If you do not have ~nix~, you can install it via
